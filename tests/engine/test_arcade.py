...
from magmacrunch.engine.arcade import (
    ENTRY_POINT_GROUP,
    ArcadeGame,
    GameInfo,
    Host,
...
def test_the_seam_depends_on_nothing():
    ...
    result = subprocess.run(
        [sys.executable, "-c",
         "import sys; import magmacrunch.engine.arcade as a; "
         "print(a.GameInfo(key='k', title='T', blurb='b').fps, "
         "'textual' in sys.modules, 'rich' in sys.modules)"],
        capture_output=True, text=True, timeout=60,
    )
    assert result.returncode == 0, result.stderr
    # textual and rich rather than tkinter, which texastoast asked about: this
    # package has no tkinter to accidentally import, and the two it does depend
    # on are the ones `magmacrunch --list` should not have to start.
