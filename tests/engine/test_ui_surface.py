...
The seam that makes this possible: widgets draw through the UISurface protocol
rather than against a concrete backend, so their logic is testable with no
terminal at all.

Inherited from texastoast, where this was the headless half of a pair — the
other half drove the same widgets over a real tkinter canvas. That half stayed
behind with the canvas, so this is now the whole of it.
"""
from magmacrunch.engine.ui.dialogue import DialogueBox
from magmacrunch.engine.ui.hud import HUD
