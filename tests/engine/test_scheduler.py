...
``GameLoop`` used to take a GUI toolkit's root window and duck-type
``after``/``after_cancel`` off it. Naming that pair as a protocol is what lets a
terminal — or these tests — drive the loop instead, and it is the reason the
loop could come across to this package at all. The proof is below: a fully
exercised game loop with no display and no sleeping.

texastoast's copy has one test more, asserting that a tkinter root satisfies
the protocol structurally. Not ported: there is no tkinter here to satisfy it,
and importing one purely to make an assertion about it would give this package
a dependency it does not otherwise have.
"""
